...
def isValidWord(word, hand, wordList):
    ...
    """
    assert not word == "", 'not valid word'
    if word in wordList:
        new_hand = {}
        for h in hand:
            new_hand[h] = hand[h]
        for w in word:
            if w not in hand.keys():
                return False
...
def playGame(wordList):
    ...
    hand = {}
    while True:
       init = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
       if init == 'e':
           break
       elif init == 'n':
           n = HAND_SIZE
